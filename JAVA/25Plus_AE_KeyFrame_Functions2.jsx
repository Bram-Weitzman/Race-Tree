...
function loadFile(lapNum, Callback){
    var lapFile = new File("F:/AE Scripts/RaceTree/Laps/Lap_" + lapNum + ".json");
    lapFile.open("r");
    var lapData = lapFile.read();
    $.writeln("Lap " + lapNum + " File Loaded")
    lapFile.close();    
...
function createKeyframe(timeOfDay, layerName, comp, position, callback) {
    ...
    if (/^[0-9]+$/.test(layerName)) {
        if (activeComp && activeComp instanceof CompItem) {
            ...
            // sort X if even or odd number
            var X = 0;
            myPosition = parseInt(position.slice(1));
            if (myPosition % 2 === 0){
                $.writeln("Position is an even number" + myPosition);
                var Y = (parseInt(position.slice(1))); // Extract Y position from the position string
                Y = (35 * Y) + 390;
                var prop = layer.property("Transform").property("Position");
                X = 475;
            } else {
                $.writeln("Position is an odd number" + myPosition);
                var Y = parseInt(position.slice(1)); // Extract Y position from the position string
                Y = (35 * Y) + 415;
                var prop = layer.property("Transform").property("Position");
                X = 200;
